...
const EmployeeExpenses = () => {
  ...
  const [form, setForm] = useState({ category: '', amount: 0, description: '' });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setShowModal(false);
      setForm({ category: '', amount: 0, description: '' });
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Submit and track expenses</p>
        </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/20 backdrop-blur-sm" onClick={() => setShowModal(false)}>
          <div className="w-full max-w-md rounded-xl border bg-card p-6 shadow-xl" onClick={e => e.stopPropagation()}>
            <h3 className="mb-4 text-lg font-semibold text-foreground">Submit Expense</h3>
            ...
            <div className="mt-6 flex gap-3 justify-end">
              ...
              <button onClick={handleSubmit} className="rounded-lg bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90">Submit</button>
            </div>
          </div>
        </div>
      )}
